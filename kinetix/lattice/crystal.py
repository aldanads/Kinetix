...
# =============================================================================
# Prepare clusters for boundary conditions
# =============================================================================                
    def prepare_clusters_for_bcs(self):
      """
      Prepare clusters for boundary condition calculation.
      
      Handles MPI internally: only rank 0 prepares clusters (has updated kMC state),
      then broadcasts results to all ranks automatically.
      
      Returns:
      --------
      clusters : dict
          Dictionary of Cluster objects with BC information prepared (same on all ranks)
      """
      # === Step 1: Rank 0 prepares clusters, others prepare to receive
      if self.rank == 0:
        clusters = self.clusters
        for cluster in clusters.values():
          cluster.prepare_cluster_for_bcs(self.grid_crystal, self.crystal_size)
...
# =============================================================================
    ...
    def _kmc_step(self, rng):
      ...
      if not TR_catalog:
        timestep_limit = self.get_timestep_limit()
      ...
      if time_step <= timestep_limit:
        ...
        # Update system state
        self.processes(chosen_event)
        self.update_superbasin(chosen_event)
        self.track_time(time_step)
...
# =============================================================================
    ...
    def should_solve_poisson_now(self, elec_controller, tol=1e-12):
      ...
      Parameters:
      -----------
      elec_controller : ElecController
          Controller object with voltage_update_time attribute
      tol : float, optional
          Time tolerance for floating-point comparison (default: 1e-12 s)
      
      Returns:
      --------
      should_solve : bool
          True if Poisson should be solved now
      is_snapshot : bool
